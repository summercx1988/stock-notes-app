...
### Changed
- 将 `explorer` 模块从“事件纵览散点图”重建为“事件日历”双栏结构：
  - 左侧按月日历展示每日笔记数量徽标。
  - 右侧展示所选日期的笔记列表（时间倒序），保留类别/观点/操作/跟踪状态标签。
  - 列表提供“打开个股笔记”跳转，继续复用现有个股编辑链路。
- 筛选能力保留并简化为日常高频项：
  - 股票关键词、类别、观点、操作、跟踪状态。
  - 数据仍复用 `timeline:queryExplorer`，按当前月份窗口查询，降低无效数据加载。
- 顶部导航将“事件纵览”改名为“事件日历”，并下线“事件时间轴”tab 入口（底层接口保留兼容，不做硬删除）。

### Notes
- 本次先做“入口下线 + 页面重建”，未删除 `notes:getTimeline` / `timeline:updateLatestTrackingStatus` 等历史接口，便于后续按调用观测再做安全清理。

## 2026-04-04 (每日复盘与复盘分析稳定性修复)

### Fixed
- 问题 1：点击“生成今日复盘”后反馈慢、列表空白，部分场景出现渲染异常并显示压缩代码片段。
  - 修复每日复盘条目类型归并逻辑，保留 `每日总结 / 盘前复习` 分类，避免历史列表被误过滤为空。
  - 生成完成后前端先本地插入新条目再异步重载，减少“按钮已点但页面无变化”的体感延迟。
  - 增强历史详情兼容解析与内容兜底，避免弹窗能打开但正文空白。
  - 提醒配置读取改为默认值归一化，`includeSections` 缺省时自动补齐，避免初始化阶段崩溃。
  - 新增顶层 `AppErrorBoundary`，渲染异常时展示可恢复错误页，不再直接暴露压缩 bundle 片段。
- 问题 2：复盘分析页切换股票后 K 线无法刷新或无响应。
  - 切换股票/查询条件时重置旧图状态并触发重新拉取，确保图表按当前标的刷新。
  - 行情拉取增加超时保护，并避免缓存空 K 线响应，防止一次失败导致后续持续空白。
  - 保留失败可见反馈路径，避免界面长时间停留在“无变化但无提示”的状态。

### Verification
- `npm run build` 通过（`tsc + vite build`）。
- `npm run electron:build` 通过，已重新产出 `release` 安装包用于回归。
