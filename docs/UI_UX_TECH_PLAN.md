...
- 左栏：盯盘文档列表（显示 `股票名称+股票代码`）
- 中栏：事件流（时间倒序）
- 右栏：摘要面板（类别分布、最近记录、快捷筛选）
...
- 单股票文档只保留 front matter + 日期分组 + 事件块，不再保留顶部一级标题
- 新增事件时按 `event_time` 统一排序重写整个股票 md，保证单文档时间轴一致性
...
- `mistake-insight-engine`：后续用于错误模式归因与提醒
