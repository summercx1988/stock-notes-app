export type ViewpointDirection = '看多' | '看空' | '未知' | '震荡'
...
const STOCK_VIEWPOINT_OPTIONS: ViewpointDirection[] = ['看多', '看空', '未知', '震荡']
  ...
  calibrate(content: string, extractedTopic: any): CalibrationResult {
    ...
    if (this.topicConfig.id === 'stock') {
      ...
      if (extractedTopic.direction) {
        const validDirections = ['看多', '看空', '未知', '震荡']
